...
# -------------JOIN verarbeiten-----------------
def handle_join(name, port, addr, ip=None):
    if ip is None:
        ip = addr[0]
    port = int(port)
...
def receive_MSG(sock, config):
    while True:
        ...
        print(f"[UDP EMPFANGEN] Von {addr}: {text}")

        teile = text.split(" ")  # wichtig: nicht auf 2 begrenzen!

        if not teile:
            print("[WARNUNG] Leere Nachricht empfangen.")
            continue
        ...
        if befehl == "JOIN":
            if len(teile) >= 3:
                ...
                print(f"[DEBUG] JOIN erhalten: name={name}, port={port}, ip={ip}")
            ...
            if config.get("client", {}).get("autoreply_aktiv", False):
                ...
                sendMSG(sock, config["client"]["handle"], absender_handle, autoreply_text)

        elif befehl == "IMG" and len(teile) == 3:
            try:
                handle_IMG(sock, teile, addr)
            except Exception as e:
                print(f"[FEHLER] Bildempfang fehlgeschlagen: {e}")
            ...
            for eintrag in eintraege:
                try:
                    ...
                    print(f"[WARNUNG] Nutzer konnte nicht verarbeitet werden: {eintrag}")

        else:
            print(f"[WARNUNG] Unbekannter Befehl: {text}")
